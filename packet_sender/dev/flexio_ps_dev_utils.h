...
flexio_dev_status_t change_status(uint16_t thd_id, eu_status old_status,
                                  eu_status new_status);

void spin_on_status(uint16_t thd_id, eu_status expected_status);

void process_packet(struct flexio_dev_thread_ctx *dtctx,
                    struct dpa_thread_context *tenant);

void pp_queue(struct flexio_dev_thread_ctx *dtctx,
              struct dpa_thread_context *this_thd_ctx,
              struct flexio_dpa_dev_queue *tenant);

void send_packet(struct flexio_dev_thread_ctx *dtctx,
                 struct dpa_thread_context *this_thd_ctx);
...
// __attribute__((unused)) static struct ether_addr SRC_ADDR = { {0x02, 0x01,
// 0x01, 0x01, 0x01, 0x01} };

__attribute__((unused)) static struct ether_addr SRC_ADDR = {
    {0xa0, 0x88, 0xc2, 0x32, 0x04, 0x40}};
